...
**Codex** — 4개 명령을 Codex 스킬로 `~/.codex/skills/` 에 깐다.

```
git clone https://github.com/nalmeokdan/long-task-nalmeok
long-task-nalmeok/codex/install.sh
```

Codex를 새로 열고 `/skills` 에서 `longtask-*` 를 명시적으로 호출한다.
방법론 원본은 `methods/` 단일 소스이며, 설치 시 각 스킬로 주입된다.
